...
def generate_routing_task(self, orders_file_content_b64: str, orders_file_name: str, num_drivers: int, split_mode: str, route_date_str: str, hq_lat: float, hq_lon: float):
    """
    ...
    try:
        ...
        orders_path = base64_data_to_temp_file(orders_file_name, orders_file_content_b64)
        orders_df = order_service.load_orders(orders_path)
        ...
        time.sleep(3)
        master_df = customer_service.load_customer_master_data()

        # 3. Merge data
        self.update_state(state='PROGRESS', meta={'status':'RUNNING','message': 'Merging order data with customer data...'}) 
        time.sleep(3)
        merged_df, missing_orders = order_service.customer_details_for_orders(orders_df, master_df)

        # 4. Assign routes using OR-Tools VRP solver
        self.update_state(state='PROGRESS', meta={'status': 'Calculating routes with OR-Tools...'}) 
        time.sleep(3)
        labels, sequences = vrp_service.solve_vrp(merged_df, num_drivers, (hq_lon, hq_lat))
        merged_df["driver_index"] = labels

        #########################################
        ###                   DEBUG           ###
        home_dir = os.path.expanduser("~")
        save_dir = os.path.join(home_dir, "tmp")
        os.makedirs(save_dir, exist_ok=True)  # create ~/tmp if missing
        ...
        print(f"Saved merged_df to: {file_path}")
        ##########################################

        # 5. Save results to Supabase

        self.update_state(state='PROGRESS', meta={'status':'RUNNING','message': 'Saving results to database...'}) 
        #route_id_map = route_service.persist_routes(merged_df, route_date_str)
        #stop_service.persist_stops(merged_df, route_id_map)
        #created_route_ids = list(route_id_map.values())
        created_route_ids=[]

        # (Optional) Still save a map for the UI to display
        temp_map_dir = os.path.join(os.getcwd(), "routes_out_temp_map")
        os.makedirs(temp_map_dir, exist_ok=True)
        route_service.save_routes_map(merged_df, temp_map_dir, route_date_str, (hq_lon, hq_lat), sequences)
